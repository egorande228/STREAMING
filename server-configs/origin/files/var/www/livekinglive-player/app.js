(function () {
  ...
  function attachVideoControls(video) {
    ...
    controls.innerHTML = `
      ...
      <button class="video-big-play" type="button" aria-label="Play stream" title="Play stream">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7z"></path></svg>
      </button>
      <div class="video-bottombar">
        <button class="video-control-btn video-play-toggle" type="button" aria-label="Play" title="Play">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7z"></path></svg>
        </button>
        <span class="video-live-text">Live broadcast</span>
        <div class="video-spacer"></div>
        <button class="video-control-btn video-mute-toggle" type="button" aria-label="Mute" title="Mute">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 9v6h4l5 4V5L8 9H4z"></path><path d="M16 9a4 4 0 0 1 0 6"></path></svg>
        </button>
        <input class="video-volume" type="range" min="0" max="1" step="0.05" value="1" aria-label="Volume" />
        <button class="video-control-btn video-pip-toggle" type="button" aria-label="Picture in picture" title="Picture in picture">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5h16v14H4z"></path><path d="M13 12h5v4h-5z"></path></svg>
        </button>
        <button class="video-control-btn video-fullscreen-toggle" type="button" aria-label="Fullscreen" title="Fullscreen">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 9V4h5"></path><path d="M20 9V4h-5"></path><path d="M4 15v5h5"></path><path d="M20 15v5h-5"></path></svg>
        </button>
      </div>
    `;
    stage.appendChild(controls);

    const bigPlay = controls.querySelector('.video-big-play');
    const playToggle = controls.querySelector('.video-play-toggle');
    const muteToggle = controls.querySelector('.video-mute-toggle');
    const volume = controls.querySelector('.video-volume');
    const pipToggle = controls.querySelector('.video-pip-toggle');
    const fullscreenToggle = controls.querySelector('.video-fullscreen-toggle');

    const playIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7z"></path></svg>';
    const pauseIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 5h4v14H7z"></path><path d="M13 5h4v14h-4z"></path></svg>';
    const volumeIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 9v6h4l5 4V5L8 9H4z"></path><path d="M16 9a4 4 0 0 1 0 6"></path></svg>';
    const mutedIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 9v6h4l5 4V5L8 9H4z"></path><path d="M18 9l-4 6"></path><path d="M14 9l4 6"></path></svg>';

    function updateState() {
      const isPaused = video.paused || video.ended;
      playToggle.innerHTML = isPaused ? playIcon : pauseIcon;
      playToggle.setAttribute('aria-label', isPaused ? 'Play' : 'Pause');
      playToggle.title = isPaused ? 'Play' : 'Pause';
      bigPlay.hidden = !isPaused;
      muteToggle.innerHTML = video.muted || video.volume === 0 ? mutedIcon : volumeIcon;
    }
    ...
    if (!document.pictureInPictureEnabled || !video.requestPictureInPicture) pipToggle.hidden = true;
  ...
  function setStreams(streams, title) {
    ...
    const requestedSource = String(params.get('source') || '').trim().toLowerCase();
    const requestedLang = String(params.get('lang') || '').trim().toLowerCase();
    let selectedIndex = 0;
    if (requestedSource) {
      const exactIndex = currentStreams.findIndex((stream) => {
        const id = String(stream.id || '').toLowerCase();
        const label = String(stream.label || '').toLowerCase();
        const type = String(stream.source_type || stream.sourceType || '').toLowerCase();
        return id === requestedSource || label === requestedSource || type === requestedSource;
      });
      if (exactIndex >= 0) selectedIndex = exactIndex;
    } else if (requestedLang) {
      const langIndex = currentStreams.findIndex((stream) => String(stream.language_code || '').toLowerCase() === requestedLang);
      if (langIndex >= 0) selectedIndex = langIndex;
    }
