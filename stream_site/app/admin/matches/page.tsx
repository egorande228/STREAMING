...
function teamLabel(team: TeamLite | null | undefined, teamID: number | null | undefined): string {
...
export default function AdminMatchesPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      resetForm();
      await load();
      setMsg(editID !== null ? 'Match updated' : 'Match created');
  ...
  return (
    <div className="text-white">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-xl font-bold">Matches ({matches.length})</h1>
        <button
          onClick={() => {
            if (showForm) {
              resetForm();
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-4 grid grid-cols-2 gap-3 rounded bg-gray-800 p-4">
          <label className="col-span-2 text-sm font-semibold">{editID !== null ? 'Edit Match' : 'New Match'}</label>

          <div>
            <p className="mb-1 text-xs text-gray-400">Home Team ID</p>
            <input
          ...
          <div>
            <p className="mb-1 text-xs text-gray-400">Away Team ID</p>
            <input
          ...
          <div>
            <p className="mb-1 text-xs text-gray-400">Group ID (optional)</p>
            <input
              value={form.group_id}
          ...
          <div>
            <p className="mb-1 text-xs text-gray-400">City</p>
            <input
              value={form.city}
          ...
          <div>
            <p className="mb-1 text-xs text-gray-400">Home Score</p>
            <input
              type="number"
              min={0}
          ...
          <div>
            <p className="mb-1 text-xs text-gray-400">Away Score</p>
            <input
              type="number"
              min={0}
          ...
          <div>
            <p className="mb-1 text-xs text-gray-400">Minute (optional)</p>
            <input
              type="number"
              min={0}
              value={form.minute}
      ...
      {scoreEdit && (
        <div className="mb-4 flex flex-wrap items-center gap-3 rounded bg-gray-800 p-4">
          <span className="text-sm font-semibold">Live Score - Match #{scoreEdit.id}</span>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-700 text-gray-400">
              ...
              <th className="py-2 pr-3 text-left">Home</th>
              <th className="py-2 pr-3 text-left">Away</th>
          ...
          <tbody>
            {matches.map((m) => (
              <tr key={m.id} className="border-b border-gray-800 hover:bg-gray-900">
                ...
                <td className="py-1.5 pr-3">{teamLabel(m.home_team, m.home_team_id)}</td>
                <td className="py-1.5 pr-3">{teamLabel(m.away_team, m.away_team_id)}</td>
                <td className="py-1.5 pr-3 font-mono">
                  {m.home_score}:{m.away_score}
                </td>
                <td className="py-1.5 pr-3 text-gray-400">{m.scheduled_at ? new Date(m.scheduled_at).toLocaleString() : '-'}</td>
                <td className="py-1.5 pr-3 text-gray-400">{m.stage}</td>
