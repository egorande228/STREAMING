...
export default function AdminMirrorsPage() {
  ...
  async function handleDelete(id: number) {
    if (!confirm('Delete mirror?')) return;
    await adminApi.mirrors.delete(id);
    load();
  }

  async function handleActivate(id: number, domain: string) {
    if (!confirm(`Set ${domain} as PRIMARY mirror?\nThis will send a Telegram alert.`)) return;
  ...
  return (
    <div className="text-white">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold">Mirrors ({mirrors.length})</h1>
        ...
        <form onSubmit={handleSubmit} className="bg-gray-800 p-4 rounded mb-4 flex flex-wrap gap-3 items-end">
          ...
          <div>
            ...
            <select value={form.region} onChange={(e) => setForm({ ...form, region: e.target.value })}
              ...
              {['global','eu','us','asia','mena'].map(r => <option key={r}>{r}</option>)}
            </select>
          </div>
          <div>
          ...
          <button type="button" onClick={() => setShowForm(false)}
            className="bg-gray-600 hover:bg-gray-700 px-4 py-1.5 rounded text-sm">Cancel</button>
        </form>
      )}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-gray-400 border-b border-gray-700">
            ...
            <th className="text-left py-2 pr-4">Last Check</th>
          ...
          {mirrors.map((m) => (
            <tr key={m.id} className={`border-b border-gray-800 hover:bg-gray-900 ${m.is_primary ? 'bg-gray-900' : ''}`}>
              ...
              <td className="py-1.5 pr-4">
                {m.is_primary
                  ? <span className="px-2 py-0.5 rounded text-xs bg-yellow-700 text-yellow-100">PRIMARY</span>
                  : <span className="px-2 py-0.5 rounded text-xs bg-gray-700 text-gray-400">standby</span>}
              </td>
              <td className="py-1.5 pr-4">
              ...
              <td className="py-1.5 flex gap-1.5">
                ...
                <button onClick={() => startEdit(m)} className="text-blue-400 hover:text-blue-300 text-xs">Edit</button>
                <button onClick={() => handleDelete(m.id)} className="text-red-400 hover:text-red-300 text-xs">Del</button>
              </td>
            </tr>
          ))}
        </tbody>
